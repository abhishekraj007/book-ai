...
export const saveDraftChapter = internalMutation({
  args: {
    bookId: v.string(),
    chapterNumber: v.number(),
    title: v.string(),
  ...
  returns: v.object({ draftId: v.string() }),
  handler: async (
    ctx,
    { bookId, chapterNumber, title, content, wordCount }
  ) => {
    ...
    // Check if draft already exists for this chapter
    ...
    if (existingDraft) {
      ...
      // Create new draft
      draftId = await ctx.db.insert("draftChapters", {
        bookId: bookId as Id<"books">,
        chapterNumber,
        title,
        content,
...
export const approveDraftChapter = internalMutation({
  args: {
    draftId: v.string(),
  },
  ...
  handler: async (ctx, { draftId }) => {
    // Get the draft
    const draft = await ctx.db.get(draftId as Id<"draftChapters">);
    if (!draft) {
    ...
    const existingChapter = await ctx.db
      .query("chapters")
      .withIndex("by_book_chapter", (q) =>
        q.eq("bookId", draft.bookId).eq("chapterNumber", draft.chapterNumber)
      )
      .first();
    ...
    if (existingChapter) {
      ...
      const newVersion = existingChapter.currentVersion + 1;
      ...
      await ctx.db.insert("chapterVersions", {
        ...
        changedBy: "ai",
        changeDescription: "Approved draft chapter",
        createdAt: now,
      });
      ...
      await ctx.db.insert("chapterVersions", {
        ...
        content: draft.content,
        changedBy: "ai",
        changeDescription: "Initial generation from draft",
        createdAt: now,
      });
    }

    // Mark draft as approved and delete it
    await ctx.db.delete(draftId as Id<"draftChapters">);

    // Update book progress
    await ctx.db.patch(draft.bookId, {
      currentStep: `chapter_${draft.chapterNumber}`,
      updatedAt: now,
    });
...
export const rejectDraftChapter = internalMutation({
  args: {
    draftId: v.string(),
  ...
  handler: async (ctx, { draftId, reason }) => {
    ...
    // Mark as rejected
    await ctx.db.patch(draftId as Id<"draftChapters">, {
      status: "rejected",
      updatedAt: Date.now(),
    });
